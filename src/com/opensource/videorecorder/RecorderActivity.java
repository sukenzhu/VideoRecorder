...
public class RecorderActivity extends BaseActivity implements SurfaceHolder.Callback {
	...
	private void initView() {
		...
		mSurfaceView = (SurfaceView) findViewById(
				mResources.getIdentifier("yuninfo_sv_recorder_preview", "id", mPackageName));

		mButton = (Button) findViewById(mResources.getIdentifier("yuninfo_btn_video_record", "id", mPackageName));
		mIbtnCancel = (ImageButton) findViewById(
				mResources.getIdentifier("yuninfo_ibtn_video_cancel", "id", mPackageName));
		...
		mTvTimeCount = (TextView) findViewById(
				mResources.getIdentifier("yuninfo_tv_recorder_time_count", "id", mPackageName));
	...
	private void exit(final int resultCode, final Intent data) {
		if (mIsRecording) {
			new AlertDialog.Builder(RecorderActivity.this).setTitle("提示").setMessage("正在录制视频，是否退出？")
	...
	private void openCamera() {
		...
		try {
			...
			if (mSupportVideoSizes == null || mSupportVideoSizes.isEmpty()) { // For
																				// some
																				// device
																				// can't
																				// get
	...
	private boolean initVideoRecorder() {
			...
			try {
				...
				if (lowProfile != null && hightProfile != null) {
					...
					lowProfile.audioSampleRate = 48000 > hightProfile.audioSampleRate ? hightProfile.audioSampleRate
							: 48000;
					// lowProfile.duration = 20 > hightProfile.duration ?
					// hightProfile.duration : 20;
					// lowProfile.videoFrameRate = 20 >
					// hightProfile.videoFrameRate ? hightProfile.videoFrameRate
					// : 20;
				...
				if (mSupportVideoSizes != null && !mSupportVideoSizes.isEmpty()) {
					...
					try {
						mMediaRecorder.setVideoSize(640, 480); // Its is not on
																// android docs
																// but
		...
		mOutputFile = new File(Environment.getExternalStorageDirectory(),
				"Video_" + DateUtil.getSystemDate("yyyy_MM_dd_HHmmss") + ".mp4");
		...
		try {
			...
			if (initVideoRecorder()) {
				...
				mButton.setBackgroundResource(
						mResources.getIdentifier("yuninfo_btn_video_stop", "drawable", mPackageName));
				...
				// inform user
				mButton.setBackgroundResource(
						mResources.getIdentifier("yuninfo_btn_video_start", "drawable", mPackageName));
			}
			mTvTimeCount.setVisibility(View.VISIBLE);
			mTvTimeCount.setText("00:0" + (Config.YUNINFO_MAX_VIDEO_DURATION / 1000));
			Message msg = mHandler.obtainMessage(Config.YUNINFO_ID_TIME_COUNT, 1,
					Config.YUNINFO_MAX_VIDEO_DURATION / 1000);
